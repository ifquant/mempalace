...
use anyhow::Result;
use mempalace_rs::model::InitSummary;
use serde_json::json;

use crate::project_cli_bootstrap_support::{
    create_bootstrap_app, print_bootstrap_json, resolve_bootstrap_config,
};
...
pub async fn handle_init(
    ...
    let config = resolve_bootstrap_config(
        Some(&palace_path),
        hf_endpoint,
        human,
    ...
    let app = create_bootstrap_app(config, human, print_init_error_human, print_init_error_json)?;
